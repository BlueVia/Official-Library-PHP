	...
	if($payment) {
		...
		if (empty($_GET['oauth_verifier'])){
			try {
				...
				$payment->getRequestToken(
								177, 						// Amount to be charged.
								
								"EUR", 						// Type of currency which corresponds 
															// with the amount above
															
								"45b5f2517b8af797de864be4", // Product identifier provided by our
															// Mobile Payments Partner
															
								"bluevia", 					// Product name as registered at our 
															// Mobile Payments Partner.
															
								CALLBACK_URL, 				// Current Application URL or MSISDN for 
															// the SMS handshake
															
								true 						// Creates token cookie and redirects 
															// automatically to oAuth portal 
															// (BlueviaClient_Api_Constants::$oauth_url)
								);
			...
			try {
				...
				$access_token = $payment->getAccessToken($_GET['oauth_verifier'], $req_token);
