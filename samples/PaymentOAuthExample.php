...
$consumerKey = "XXXXXXXXXXXXXX"; //CONSUMER_KEY,
$consumerSecret = "YYYYYYYYYYYYY"; //CONSUMER_SECRET

// The First step for using the Oauth Api, is to create the Oauth object
$payment = new BV_Payment($mode,$consumerKey,$consumerSecret);
...
if (empty($_GET['oauth_verifier'])){
	try {
		...
		$serviceName='XXXXX';
		$serviceId='YYYYYYYYYYYYY';
		
		$request_token = $payment->getPaymentRequestToken(0,'EUR',$serviceName,$serviceId,$callbackURL);;
	...
	try {
		...
		$access_token = $payment->getPaymentAccessToken($_GET['oauth_verifier']);

		// Show the returned information, only if DEBUG is set to true
		$p=$payment->payment(0,'EUR');
		if (DEBUG){
			print "<h1>Oauth API</h1>";
