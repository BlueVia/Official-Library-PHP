...
class BlueviaClient_Api_Payment extends BlueviaClient_Api_Client_RPC {
    ...
    */
     ...
     * @param string $serviceID Product identifier provided by our Mobile Payments Partner
     * @param string $serviceName Product name as registered at our Mobile Payments Partner.
     * @param string $callback_url Current Application URL or MSISDN for the SMS handshake
     * @param bool $autoredirect Creates token cookie
    ...
    public function getRequestToken($amount,
    	...
    	if (!(is_null($callback_url) || empty($callback_url)))
    			if (!(Zend_Uri::check($callback_url) || $callback_url ==='oob'))
    				throw new BlueviaClient_Exception_Parameters("Invalid callback url");
    	...
    	try {    		    		
    		...
    		$requestToken = $oAuth->getRequestToken($callback_url,
    												$autoredirect,
    												$custom_payment_options,
    												$oauth_Name);
			unset($oAuth);    											
			if(!$autoredirect || empty($callback_url) || $callback_url=='oob')	
				return $requestToken;    											
    	} catch (Exception $ex) {
    		unset($oAuth);
