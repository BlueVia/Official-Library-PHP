...
include_once 'BlueviaClient/Api/MO.php';
include_once 'BlueviaClient/Api/MT.php';
include_once 'BlueviaClient/Api/SmsMO.php';
include_once 'BlueviaClient/Api/MmsMO.php';
include_once 'BlueviaClient/Api/SmsMT.php';
include_once 'BlueviaClient/Api/MmsMT.php';
...
class BlueviaClient implements ArrayAccess
{
    ...
    /**
     * @param <string> $environment [LIVE | TEST | SANDBOX]
     ...
     */
    public function  __construct($environment,$application_context = null, $log = null)
    {
        ...
        if ($environment===BlueviaClient_Api_Constants::TEST_MODE) {
        	BlueviaClient_Api_Constants::$oauth_url = BlueviaClient_Api_Constants::OAUTH_URL_TEST;
        }
        else if ($environment===BlueviaClient_Api_Constants::SANDBOX_MODE){
        	BlueviaClient_Api_Constants::$oauth_url = BlueviaClient_Api_Constants::OAUTH_URL_TEST;
        	BlueviaClient_Api_Constants::$environment  = BlueviaClient_Api_Constants::ENVIRONMENT_SANDBOX;
        }
     	else if ($environment!=BlueviaClient_Api_Constants::LIVE_MODE){
			throw new BlueviaClient_Exception_Parameters ('Invalid environment');
        }
        ...
        $this->_options['oauthUrl']     = BlueviaClient_Api_Constants::$oauth_url;
    ...
    public function getService($name)
    {
        ...
        $twolegged_classes = array(
        	...
        	'BlueviaClient_Api_Mmsmo',
        	'BlueviaClient_Api_Smsmo'
        );
        ...
        if ($class=='BlueviaClient_Api_Mmsmo' || $class=='BlueviaClient_Api_Smsmo'){
        	if (!(empty($this->_context['user']['token_access']) ||
                    empty($this->_context['user']['token_secret']))) {
                    	throw new BlueviaClient_Exception_Parameters($name." is a two legged API");
                    }
        }
            ...
            $header_mods = array())
    {
        ...
        // file attachments
        if (!empty($files)) {
            $client->setFileUpload('message', 'root-fields', $msg_body, 'application/json;charset=UTF-8');
            $client->setHeaders('Content-transfer-encoding: binary');
           
            // mms body inclusion as attachment
            if(!empty($mms_body))
            	$client->setFileUpload('textBody.txt', 'body', $mms_body, 'text/plain;charset=UTF-8');            
